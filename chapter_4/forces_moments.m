...
function out = forces_moments(x, delta, wind, P)
    ...
    R_vb = R_bv(phi, theta, psi)';
    gust_wind_v = R_vb*wind(4:6);
    w_n = w_ns + gust_wind_v(1);
    w_e = w_es + gust_wind_v(2);
    w_d = w_ds + gust_wind_v(3);
    
    % compute relative speed
    steady_wind_b = R_bv(phi, theta, psi)*wind(1:3); %steady wind defined in body frame
    ...
    Force(1) =  -P.mass*P.gravity*sin(theta) + 0.5*P.rho*Va^2*P.S_wing*(C_x(alpha, P) + C_xq(alpha, P)*P.c/(2*Va)*q + C_x_delta_e(alpha, P)*delta_e) + 0.5*P.rho*P.S_prop*P.C_prop*((P.k_motor*delta_t)^2-Va^2);
    Force(2) =  P.mass*P.gravity*cos(theta)*sin(phi) + 0.5*P.rho*Va^2*P.S_wing*(P.C_Y_0 + P.C_Y_beta*beta + P.C_Y_p*P.b/(2*Va)*p + P.C_Y_r*P.b/(2*Va)*r + P.C_Y_delta_a*delta_a + P.C_Y_delta_r*delta_r);
    Force(3) =  P.mass*P.gravity*cos(theta)*cos(phi) + 0.5*P.rho*Va^2*P.S_wing*(C_z(alpha, P) + C_zq(alpha, P)*P.c/(2*Va)*q + C_z_delta_e(alpha, P)*delta_e);
